...
public class Controller {
    ...
    private static final byte[] rotateBits = {
    ...
    private static long permute(byte[] table, int widthValue, long src) {
        long dst = 0;
        for (int i = 0; i < table.length; i++) {
            int srcPos = widthValue - table[i];
    ...
    private static long[] createKeysVariation(long key) {
    ...
    private static long encryptBlock(long m, long key) {
        ...
        long subkeys[] = createKeysVariation(key);
    ...
    public static String encryptLaunch(String inputtedMessage, String inputtedPassword ) {

        inputtedMessage = StringToBinaryFormat(inputtedMessage);

        String [] messageByWords = inputtedMessage.split("\\s");
        inputtedPassword = StringToBinaryFormat(inputtedPassword);


        String result = "";

        for (int i=0; i<messageByWords.length; i++) {
            byte [] userData = parseBytes(messageByWords[i]);
            byte [] userKeyData = parseBytes(inputtedPassword);
            userData = encrypt(userData, userKeyData);
            result +=hex(userData);
        }
    ...
    public static boolean checkPassword(String userPassword){
        ...
        boolean result = userPassword.matches(pattern);
        return result;
    }
}
